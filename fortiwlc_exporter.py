...
productionapiarray = ['https://wlc1.anso.arnes.si/api/v2/monitor/wifi/managed_ap/select/?vdom=root&access_token=kQ0bg3jg6pfn19kr4GdgzGx41dmk9w',
...
def ssid_count(path,ap):
    ...
    '''if len(ssid0) > 0:
        if 'SGR_' not in ssid0[0]:
            path['ssids'].add(ssid0[0])
        else:
            if ssid0[0] not in ssidgroupset:
                ssidgroupset.add(ssid0[0])
                for ssidgroup in datassid:
                    if ssidgroup['name'] in ssidgroupset:
                        for x in ssidgroup['vaps']:
                            path['ssids'].add(x['name'])'''
    '''if len(ssid1) >= 1:
        if 'SGR_' not in ssid1[0]:
            path['ssids'].add(ssid1[0])
        else:
            ssidgroupset.add(ssid1[0])

    for ssidgroup in datassid:
        if ssidgroup['name'] in ssidgroupset:
            for x in ssidgroup['vaps']:
                path['ssids'].add(x['name'])
    print(path['ssids'], id(path['ssids']))
    path['ssid_count'] = len(path['ssids'])'''
        #print(ssidgroup)
...
def ap_campus_counter(z):
    ...
    for ap in data:
        ...
        if campus != '':
            ...
            per_model_write(camp[campus]['ap_count']['per_model'],ap)
            ...
            path['total'] += 1
            per_model_write(path['per_model'],ap)
...
def wlc_write(ap,z):
    ...
    per_model_write(wlc['ap_count']['per_model'],ap)
...
#print(json.dumps(maindict, indent = 4, sort_keys = True))
